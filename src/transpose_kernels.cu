...
static void __global__ trans_zxy_to_yzx_kernel(float2* input, float2* output, domain_t domain)
{
   __shared__ float2 scratch[THREADSPERBLOCK_IN][THREADSPERBLOCK_IN+1];
  int k_in = blockIdx.x*blockDim.x + threadIdx.x;
  int j_in = blockIdx.y*blockDim.y + threadIdx.y;
...
static void __global__ trans_yzx_to_zxy_kernel(float2* input, float2* output, domain_t domain)
{
   __shared__ float2 scratch[THREADSPERBLOCK_IN][THREADSPERBLOCK_IN+1];
  int i    = blockIdx.z;
  ...
  int index_ot = i*NY*NZ + j_ot*NY + k_ot;
  if(k_ot<NY && j_ot<NZ){
    scratch[threadIdx.x][threadIdx.y] = input[index_ot];
  }
  __syncthreads();
  int k_in = blockIdx.x*blockDim.x + threadIdx.x;
  int j_in = blockIdx.y*blockDim.y + threadIdx.y;
  int index_in = j_in*gridDim.z*NZ + i*NZ + k_in;
  if(j_in<NY && k_in<NZ){
    output[index_in] = scratch[threadIdx.y][threadIdx.x];
  }
}


static void __global__ trans_zyx_to_yzx_kernel(float2* input, float2* output, domain_t domain)
{
...
static void __global__ trans_yzx_to_zyx_kernel(float2* input, float2* output, domain_t domain)
{
   __shared__ float2 scratch[THREADSPERBLOCK_IN][THREADSPERBLOCK_IN+1];
/*
...
extern void trans_zxy_to_yzx(float2* input, float2* output,cudaStream_t stream,domain_t domain)
{
        ...
        trans_zxy_to_yzx_kernel<<<blocksPerGrid,threadsPerBlock,0,stream>>>(input,output,domain);
        //kernelCheck(RET,"rk_substep",1);
...
extern void trans_zyx_to_yzx(float2* input, float2* output,cudaStream_t stream,domain_t domain)
{
        ...
        trans_zyx_to_yzx_kernel<<<blocksPerGrid,threadsPerBlock,0,stream>>>(input,output,domain);
        //kernelCheck(RET,"rk_substep",1);
...
extern void trans_yzx_to_zyx(float2* input, float2* output,cudaStream_t stream,domain_t domain)
{
        ...
        trans_yzx_to_zyx_kernel<<<blocksPerGrid,threadsPerBlock,0,stream>>>(input,output,domain);
        //kernelCheck(RET,"rk_substep",1);
