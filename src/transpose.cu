...
START_RANGE("transpose_A",23)
  ...
  CHECK_CUBLAS( cublasCgeam(cublasHandle,CUBLAS_OP_T,CUBLAS_OP_T,NX*NZ,NY,alpha,(const float2*)u_1,NY,0,0,NY,u_2,NX*NZ) );
...
START_RANGE("transpose_B",24)
  ...
  CHECK_CUBLAS( cublasCgeam(cublasHandle,CUBLAS_OP_T,CUBLAS_OP_T,NY,NX*NZ,alpha,(const float2*)u_1,NX*NZ,0,0,NX*NZ,u_2,NY) );
  //printf("\n%f,%f",alpha[0].x,alpha[0].y);
END_RANGE
  return;
