...
extern void setHemholzt(domain_t domain){
  ...
  for(int i=0;i<NXSIZE;i++){
    ...
    float kx=(i+domain.iglobal)<NX/2 ? (float)(i+domain.iglobal) : (float)(i+domain.iglobal)-(float)NX ;
    //kx=0;
    for(int k=0;k<NZ;k++){
  ...
  cudaCheck(cudaMemcpy(udiag,udia_host,SIZE,cudaMemcpyHostToDevice),domain,"C");
  cudaCheck(cudaMemcpy(cdiag,cdia_host,SIZE,cudaMemcpyHostToDevice),domain,"C");
  cudaCheck(cudaMemcpy(ldiag,ldia_host,SIZE,cudaMemcpyHostToDevice),domain,"C");
